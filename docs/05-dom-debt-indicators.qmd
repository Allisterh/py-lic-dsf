...
**Edit Dom sheets in Excel**, save, then `load_domestic`.
...
| `load_domestic` (`lic_dsf.load`) | Dom sheet blocks → `DomesticDebtInputs` |

Public DSA and Macro already stitch domestic stocks into public ratios when
you build via `load_core`; use Dom when you need Dom-sheet indicators
...
`load_domestic` pulls Baseline public/external ratios, Macro stocks/flows, and
Input 7 domestic borrowing assumptions (see demo).
...
— Public DSA after `load_core`; domestic inputs via `load_domestic`.
